...
> Сессия: ревизия статуса + планирование следующих задач
...
- [x] Проверить форму: жүз → ру → карточка → 7 предков → submit → SVG дерево ✅
- [x] Проверить Canvas share/download (все 4 цветовые схемы) ✅
- [x] Проверить переключение языков `/kk` ↔ `/ru` ✅
- [x] Проверить localStorage (жүз, ру, язык сохраняются между сессиями) ✅
- [x] Responsive: 900px, 768px, 500px ✅
- [x] Мобильная навигация (бургер-меню) ✅

### Приоритет 2 — Деплой и инфраструктура
- [x] Деплой на Vercel (или другой хостинг) — подключить GitHub repo ✅
- [x] Привязать домен skezire.kz ✅
- [x] CI/CD: автодеплой при push в main ✅
- [x] Закоммитить и запушить Dockerfile + .dockerignore + обновлённый next.config.ts ✅

### Приоритет 3 — Улучшения
- [x] OG-изображение для соцсетей ✅ (динамический opengraph-image.tsx, 1200×630)
- [x] ~~QR-код на PNG картинке~~ — убрано из плана
- [ ] Варианты формата: Story (1080×1920), квадрат (1080×1080)
- [ ] Удалить `_legacy/` после полной проверки

### Приоритет 4 — Новые фичи
- [x] ~~AI «История рода»~~ — убрано из проекта (удалён код)
- [x] ~~PDF-экспорт дерева~~ — убрано из плана
- [ ] Заказ картины шежіре (форма заказа печатной/canvas-версии дерева)
- [ ] PWA: service worker, manifest.json, офлайн-доступ
- [x] Аналитика ✅ (Яндекс Метрика)

### Приоритет 5 — Энциклопедия (отдельные страницы)
- [x] Хаб: 4 карточки жузов `/[locale]/encyclopedia` ✅
- [x] Страница жуза `/[locale]/encyclopedia/[zhuzId]` ✅
- [x] Страница рода `/[locale]/encyclopedia/[zhuzId]/[tribeId]` ✅
- [x] Компоненты: HubCard, ZhuzSection, TribeCardEnc, TribeDetail, TribeTabs, Breadcrumb, Pager ✅
- [x] Переводы enc.* (27 ключей, kk + ru) ✅
- [x] Sitemap (~106 URL) ✅
- [x] SEO metadata + JSON-LD на всех уровнях ✅
