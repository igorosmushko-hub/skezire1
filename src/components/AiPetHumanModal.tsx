...
export function AiPetHumanModal({ open, onClose }: Props) {
  const t = useTranslations('ai.pet.modal');
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      if (status === 'succeeded' && output?.[0]) {
        ...
        setProgress(100);
        setStep('result');
        aiGenerateSuccess('pet-humanize');
      } else {
        showToast(t('error'));
        setStep('preview');
      }
  ...
  const handleDownload = useCallback(async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = 'shezhire-pet-human.jpg';
        ...
        {step === 'result' && resultUrl && (
          <div className="ai-past-step">
            ...
            <div className="ai-past-compare">
              ...
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={resultUrl} alt={isKk ? 'AI нәтиже — питомец адамға' : 'AI результат — питомец в человека'} className="ai-past-img-result" />
            </div>
            <div className="ai-past-actions">
              ...
              <a
                ...
                href={`/${locale}/order/canvas?image=${encodeURIComponent(resultUrl)}&type=pet-humanize`}
              >
