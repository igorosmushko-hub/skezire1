...
export function AiAncestorModal({ open, onClose }: Props) {
  const t = useTranslations('ai.gm.modal');
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      const createRes = await fetch('/api/ai/generate', {
        ...
        body: JSON.stringify({ imageBase64, gender, type: 'ancestor', aspectRatio }),
      });
  ...
  const handleDownload = useCallback(async () => {
    ...
    try {
      ...
      const file = new File([blob], 'shezhire-ancestor-young.jpg', { type: 'image/jpeg' });
      ...
      a.href = url;
      a.download = 'shezhire-ancestor-young.jpg';
        ...
        {step === 'result' && resultUrl && (
          <div className="ai-past-step">
            ...
            <div className="ai-past-compare">
              ...
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={resultUrl} alt={isKk ? 'AI нәтиже — ата-баба фотосы' : 'AI результат — фото предка'} className="ai-past-img-result" />
            </div>
