  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      const createRes = await fetch('/api/ai/generate', {
        ...
        body: JSON.stringify({ imageBase64, gender, type: 'action-figure', aspectRatio }),
      });
      ...
      if (status === 'succeeded' && output?.[0]) {
        ...
        setProgress(100);
        setStep('result');
        aiGenerateSuccess('action-figure');
      } else {
        showToast(t('error'));
        setStep('preview');
      }
        ...
        {step === 'result' && resultUrl && (
          <div className="ai-past-step">
            ...
            <div className="ai-past-compare">
              ...
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={resultUrl} alt={isKk ? 'AI нәтиже — экшн-фигурка' : 'AI результат — экшн-фигурка'} className="ai-past-img-result" />
            </div>
